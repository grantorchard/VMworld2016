...
def doEntireThing():
    ...
    networkPathLabel = 'Pray to the Demo Gods'
...
def postNetworkProfile():
    ...
    with open('profile.json', 'r+') as sfile:
...
def getComputeResourceIds():
    ...
    for item in body[u'values']:
        if item['label'] == 'Cluster Site A (vCenter)':
            ...
            print 'Compute Resource with ID ' + item['underlyingValue']['id'] + ' found with the required Network Path.'
...
def getNetworkPathId():
    ...
    req = requests.post(url, headers = auth_headers, verify = False, data='{"text":"Element","dependencyValues":{"entries":[{"key":"computeResource","value":{"type":"entityRef","componentId":null,"classId":"ComputeResource","id":"8f0e4c3e-f8d2-4ecb-aabb-fb1b484b5ba9","label":"Cluster Site A (vCenter)"}}]},"associateValue":null}')
    body = req.json()
    #print body
    for item in body[u'values']:
